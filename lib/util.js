...
function parseGitPull(result, directory) {
  ...
  if (range) {
    var logs = extras.get(`git -C ./${directory} log ${range}`)
    var authors = [
      ...new Set(
...
function getChanges(status) {
  ...
  var untracked = isUntracked
    ? status
        ...
        .flatMap((s) => s.split('\t'))
        .filter(Boolean)
        .filter((s) => !s.includes('nothing added to commit'))
        .filter((s) => !s.includes('no changes added to commit'))
    : undefined
